...
std::string AeDecelerate::m_alias = "AeDecelerate";

AeDecelerate::AeDecelerate(float x_factor, float y_factor, std::string x_name, std::string y_name) :
EventContainer(),
m_x_factor{x_factor},
m_y_factor{y_factor},
...
AeDecelerate* AeDecelerate::create(float x_factor, float y_factor, std::string x_name, std::string y_name) {
    AeDecelerate temp(x_factor, y_factor, x_name, y_name);
 ...
 */
tinyxml2::XMLError AeDecelerate::parse(tinyxml2::XMLElement* source, MapData& map, std::pair<std::string, ActorEvent*>& entry) const{
    ...
    ActorEvent* event = create(x_factor,y_factor, x_speed_name, y_speed_name);
